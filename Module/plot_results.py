...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Skip generating the diagnostics_solution_N.csv report for the selected solution "
             "(generated by default, alongside the rule curve/climatology/etc., so this run's "
             "plots and diagnostics always describe the exact same candidate).",
    )
    ...
    if solution_index is not None:
        ...
        if not args.no_diagnostics:
            # Generate the diagnostics report/CSV for the SAME solution this run
            # selected -- avoids the two-command coordination problem where
            # diagnostics.py and plot_results.py need matching --min-reliability/
            # --solution-index to stay in sync; this way one command guarantees it.
            diag_csv_path = Path(output_dir) / f"diagnostics_solution_{solution_index}.csv"
            report = diagnose(data, categories, x)
            report.to_csv(diag_csv_path, extra_metadata={
